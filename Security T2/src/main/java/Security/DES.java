...
public class DES {
    ...
    private String xor(String a, String b) {
        // DONE: Implement bitwise XOR between strings 'a' and 'b'.
        ...
        String result = anum.xor(bnum).toString(2);
        int resLength = result.length();
        int ogLength = a.length();
        if (resLength != ogLength) {
            result = "0".repeat(ogLength - resLength) + result;
        }
        return result;
    }
    ...
    private static String[] generateSubKeys(String keyBin) {
        ...
        // DONE: Apply PC-1 to the 64-bit key to get a 56-bit key
        String keyPC1 = permute(keyBin, PC1);

        // DONE: Split the 56-bit key into two halves
        String leftKeyPC1 = keyPC1.substring(0, 28);
        String rightKeyPC1 = keyPC1.substring(28, 56);

        // DONE: Generate 16 subkeys by shifting and applying PC-2
        String left = leftKeyPC1;
        String right = rightKeyPC1;

        for (int i = 0; i < 16; i++) {
            left = leftShift(left, SHIFTS[i]);
            right = leftShift(right, SHIFTS[i]);
            subkeys[i] = permute(left + right, PC2);
        }
    ...
    private String feistel(String R, String subKey) {
        // DONE: Expand R to 48 bits using E-table
        R = permute(R, E);
        // DONE: XOR the expanded R with the subkey
        R = xor(R, subKey);
        // WIP: Divide the xored result into 8 blocks and apply S-box substitution
        ...
        // DONE: Apply permutation P to the substituted output
        String cipherBinary = permute(substituted.toString(), P);
        return cipherBinary;
    }
